...
### Call a unified API
```kotlin
// steamClient.grpc is SteamGrpcClients that provides autogenerated accessors for most Steam APIs
val result: CPlayer_GetOwnedGames_Response = steamClient.grpc.player.GetOwnedGames().executeSteam(
    ...
    anonymous = false, // ignore authorization
    web = false, // use Web API if required (like 2FA API)
)
```
---
