...
class News internal constructor(
    ...
    suspend fun getCommunityHub(
        ...
        return steamClient.externalWebApi.ajaxGetTyped<CommunityHubResponse>(
            path = listOf("library", "appcommunityfeed", appId.id.toString()),
            parameters = mapOf(
                "p" to "1",
                "filterLanguage" to steamClient.config.language.vdfName,
                "languageTag" to steamClient.config.language.vdfName,
    ...
    suspend fun getUserNews(
        ...
        val newsProto = steamClient.webApi.execute(
